  ...
  return (
    ...
    >
      ...
      <div className="left-column">
        ...
        >
          Your go-to test portal for placement training and company drives exams. Access a wide range of scheduled tests aimed in enhancing your knowledge, aptitude, and core competencies.
        </p>
        ...
        >
          {Array.from({ length: totalQuestion }, (_, index) => (
            <a href={`#${index}`} style={{ color: 'white', textDecoration: 'none' }}>
              <button
      ...
      <div className="right-column" style={{ overflowY: "hidden" }}>
        {question.map((question, index) => (
          <div id={index} key={index} style={{ height: "100vh", padding: '50px 0' }}>
            <p
              style={{
                textAlign: "center",
                color: "white",
            ...
            {question.options.map((option, optionIndex) => (
              <>
                <ul style={{ listStyleType: "none", padding: 0 }}>
                  ...
                  >
                    {question.multipleselect === 1 ? (
                      <label style={{ marginRight: "10px", width: '100%', display: 'flex', justifyContent: 'space-between', }}>
                        <input
                          type="checkbox"
                          name={`option-${index}`}
                          value={option}
                ...
                {optionIndex === 3 ? (
                  <div className="questionButtons">
                    <button className="b1" onClick={() => { handleClear({ index }) }}>Clear Response</button>
                    <button className="b2">Mark for Review & Next</button>
                    <button className="b4">Save & Mark for review</button>
                    {index !== totalQuestion - 1 ? (
                      <button className="b3"><a href={`#${index + 1}`} style={{ color: 'white', textDecoration: 'none' }}>Save & Next</a></button>
                    )
                      :
                      ""
                    }
